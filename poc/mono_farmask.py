...
def edge_unknown_band(depth: np.ndarray, width: int = 7) -> np.ndarray:
...
def process(pipe, path: Path, out_dir: Path, tag: str, far_pct: float):
    ...
    unknown = edge_unknown_band(inv, width=9) & ~far
    near = ~far & ~unknown
...
def main():
    ...
    for i, p in enumerate(args.images):
        inv, _ = process(pipe, p, args.out, tag=f"img{i}", far_pct=args.far_pct)
        invs.append(inv)
